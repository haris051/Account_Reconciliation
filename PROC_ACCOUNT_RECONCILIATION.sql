...
DELIMITER $$
CREATE PROCEDURE `PROC_ACCOUNT_RECONCILIATION`( P_ENTRY_DATE TEXT,
											   P_ACCOUNT_ID INT,
											   P_COMPANY_ID INT,
											   P_FORM TEXT )
BEGIN
	...
	-- ================ Getting Account Type value using in below query ================
    ...
    -- ================ Data from all accounting tables for Reconciliation ================
	...
	THEN
			...
			where  (
			  ...
			  select A.id,
					...
					case 
						...
						WHEN A.GL_FLAG = 101 OR A.GL_FLAG = 26 OR A.GL_FLAG = 23 OR A.GL_FLAG = 201 OR A.GL_FLAG = 102 OR A.GL_FLAG = 203 OR A.GL_FLAG = 103 OR A.GL_FLAG = 104 OR A.GL_FLAG = 105 OR A.GL_FLAG = 106 OR A.GL_FLAG = 5553 OR A.GL_FLAG=5554 THEN 'Vendor Payments'
						WHEN A.GL_FLAG = 107 OR A.GL_FLAG = 29 OR A.GL_FLAG = 28 OR A.GL_FLAG = 204 OR A.GL_FLAG = 205 OR A.GL_FLAG = 108 OR A.GL_FLAG = 109 OR A.GL_FLAG = 110 OR A.GL_FLAG = 111 OR A.GL_FLAG = 113 OR A.GL_FLAG = 114 OR A.GL_FLAG = 112 OR A.GL_FLAG = 5552 OR A.GL_FLAG =5551 THEN 'Customer Receipts'
						WHEN A.GL_FLAG = 115 OR A.GL_FLAG = 89 OR A.GL_FLAG = 90 OR A.GL_FLAG = 116 OR A.GL_FLAG = 117 THEN 'Repair Charges'
					End as Form_Type,
					case 
						when Convert(A.Form_Date,Date) <= Convert(Date_To,Date) and Convert(A.Reconcile_Date,Date) > Convert(Date_To,Date) and A.IS_CONFLICTED='Y'  then 'N'
						when Convert(A.FORM_DATE,Date) <= Convert(Date_To,Date) and A.IS_CONFLICTED = 'N' then 'N'
			...
			ORDER BY IS_CONFLICTED DESC, FORM_DATE ASC;
	END IF;
    ...
    -- ================ Update accounting tables when deleting the Reconciliation ================
										
	IF P_FORM = 'Delete'
	THEN 
		START TRANSACTION;
			update payments_accounting SET IS_CONFLICTED = 'N',Reconcile_Date = '1999-01-01 00:00:00' 
			where GL_ACC_ID = P_ACCOUNT_ID and Company_Id = P_COMPANY_ID and convert(Reconcile_Date,date) >=Convert(Date_From,Date)  and convert(Reconcile_Date,date) <=Convert(Date_To,Date);
        
			update Purchase_Accounting SET IS_CONFLICTED = 'N',Reconcile_Date = '1999-01-01 00:00:00' 
            where GL_ACC_ID = P_ACCOUNT_ID and Company_Id = P_COMPANY_ID and convert(Reconcile_Date,date) >=Convert(Date_From,Date)  and convert(Reconcile_Date,date) <=Convert(Date_To,Date);
            
			update Sales_Accounting SET IS_CONFLICTED = 'N',Reconcile_Date = '1999-01-01 00:00:00' 
            where GL_ACC_ID = P_ACCOUNT_ID and Company_Id = P_COMPANY_ID and convert(Reconcile_Date,date) >=Convert(Date_From,Date)  and convert(Reconcile_Date,date) <=Convert(Date_To,Date);
            
			update stock_accounting SET IS_CONFLICTED = 'N',Reconcile_Date = '1999-01-01 00:00:00' 
            where GL_ACC_ID = P_ACCOUNT_ID and Company_Id = P_COMPANY_ID and convert(Reconcile_Date,date) >=Convert(Date_From,Date)  and convert(Reconcile_Date,date) <=Convert(Date_To,Date);
            
			update Repair_Accounting SET IS_CONFLICTED = 'N',Reconcile_Date = '1999-01-01 00:00:00' 
            where GL_ACC_ID = P_ACCOUNT_ID and Company_Id = P_COMPANY_ID and convert(Reconcile_Date,date) >=Convert(Date_From,Date)  and convert(Reconcile_Date,date) <=Convert(Date_To,Date);
            
			update Adjustment_Accounting SET IS_CONFLICTED = 'N',Reconcile_Date = '1999-01-01 00:00:00' 
            where GL_ACC_ID = P_ACCOUNT_ID and Company_Id = P_COMPANY_ID and convert(Reconcile_Date,date) >=Convert(Date_From,Date)  and convert(Reconcile_Date,date) <=Convert(Date_To,Date);
		COMMIT;
	END IF;
    ...
    -- ================ Getting last reconciled difference value ================
